...
- No write-privilege probe: `Connection::probe_write_privilege` intentionally stays at the trait default (`WritePrivilege::Unknown`), since the driver already rejects every mutating statement at the wire layer regardless of the connected role's actual grants.
